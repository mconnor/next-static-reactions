...
export async function getStaticProps() {
	...
	const res = await fetch('https://api.github.com/graphql', {
		...
		body: JSON.stringify({
			query: `query {
        repository(owner:"mconnor", name:"next-static-reactions") {
          issue(number:1) {
            reactionGroups {
              content
              users(first: 0) {
	...
	return (
		<div className="container">
			...
			<main>
				...
				<h3>
					Reactions on <a href="https://github.com/chibicode/reactions/issues/1">this GitHub issue</a>
					:
				</h3>
				<div className="line">
				...
				<div>
					...
					<a href="https://vercel.com/docs/v2/edge-network/overview">Vercel’s Edge Network</a> (CDN).
					...
					(released in <a href="https://nextjs.org/blog/next-9-5">Next.js 9.5</a>). Here’s how it works:
				</div>
				<ol>
					...
					<li>
						Later, when another request comes in <strong>after the defined timeout is exceeded</strong>: (1)
						The statically generated page is served, and (2){' '}
						<strong>
							Next.js generates a new version of the page in the background and updates the static page
							for *future* requests
						</strong>
						.
					</li>
					<li>
						Later, when another request comes in <strong>after the regeneration is done</strong>: The
						updated static page is served.
					</li>
					<li>
						This allows Incremental Static Regeneration on a per-page basis without rebuilding the full app.
						It’ll always be fast because users will always get a static response.{' '}
						<a href="https://nextjs.org/docs/basic-features/data-fetching#incremental-static-regeneration">
							Learn more here
						</a>
						.
				...
				<div>
					...
					<a href="https://github.com/chibicode/reactions/blob/master/pages/index.js">pages/index.js</a> -
					`getStaticProps()` fetches the data during static generation, and `revalidate` specifies the
					timeout.
				</div>
			</main>
		</div>
